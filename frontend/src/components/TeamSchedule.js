...
function TeamSchedule({ teamId }) {
  ...
  const fetchTeamTasks = async () => {
    // teamId가 유효하지 않으면 요청하지 않음
    if (!teamId) return;

    try {
      // api 모듈 사용 및 올바른 경로(/api/schedule/task/list/)로 수정
      const response = await api.get(`/api/schedule/task/list/?team_id=${teamId}`);
      ...
      taskList.forEach((task) => {
        ...
        // 백엔드 필드명이 end_date인지 확인 필요 (보내주신 코드 기준 end_date_due_date가 있어서 그대로 유지하되, 없다면 end_date로 수정 필요)
        // 보통 Task 모델에는 end_date가 있으므로 안전하게 end_date 사용 고려
        const dateValue = task.end_date || task.end_date_due_date;
