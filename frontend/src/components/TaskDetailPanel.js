...
function TaskDetailPanel({ task, projectId, onClose, onUpdate,  fetchTasks  }) {
 ...
 // 댓글 작성
  ...
  // 상태 변경 처리 (개선됨 - 모든 상태 변경에 대해 상위 업무 자동 업데이트)
  const handleStatusChange = async (e) => {
    ...
    const oldStatusLabel = status; // 이전 상태 저장
    ...
    try {
      console.log(`📤 상태 변경 요청: ${oldStatusLabel} → ${newStatusLabel}`);
      
      // ✅ 백엔드 응답에서 auto_updated 정보 받기
      ...
      // ✅ [핵심] 자동 업데이트된 상위 업무들 처리 (모든 상태 변경에 대해)
      const autoUpdated = response.data?.auto_updated?.parents || [];
      
      if (autoUpdated.length > 0) {
        console.log(`✅ 상위 업무 ${autoUpdated.length}개 자동 업데이트됨:`, autoUpdated);
        
        // 각 상위 업무 정보를 다시 fetch해서 화면에 반영
        for (const parentId of autoUpdated) {
          try {
            const parentRes = await axios.get(
              `http://127.0.0.1:8000/api/tasks/${parentId}/`
            );
            ...
            const parentStatusLabel = 
              parentData.status == 0 ? "요청" :
              parentData.status == 1 ? "진행" :
              parentData.status == 2 ? "피드백" :
              parentData.status == 3 ? "완료" : "보류";
            
            console.log(`  ├─ 상위 업무 ${parentId} 갱신: ${parentStatusLabel}`);
            
            // 부모 컴포넌트(TaskPage)에 업데이트 알림
            if (onUpdate) {
              onUpdate({
                task_id: parentData.task_id,
                status: parentStatusLabel,
                assignee: parentData.assignee,
                start_date: parentData.start_date,
                end_date: parentData.end_date,
                task_name: parentData.task_name,
                parent_task_id: parentData.parent_task
              });
            }
          } catch (err) {
            console.error(`❌ 상위 업무 ${parentId} 갱신 실패:`, err);
          }
        }
        
        console.log("✅ 모든 상위 업무 갱신 완료");
      } else {
        console.log("ℹ️ 자동 업데이트된 상위 업무 없음");
      }
  ...
  const handleAiGuide = async () => {
    try {
      await axios.post(
        `http://127.0.0.1:8000/api/tasks/${task.task_id}/generate-ai-guide/`, 
        {}
      );
