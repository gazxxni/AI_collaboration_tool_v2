...
function TeamFinder() {
  ...
  const handleChat = async post => {
    try {
      // ✅ [수정됨] 올바른 URL: /api/dm_rooms/create/ (chat 제거, create 추가)
      const resp = await axios.post(
        "http://127.0.0.1:8000/api/dm_rooms/create/",
        { user_id: userId, target_id: post.author_id },
        { withCredentials: true }
      );
      const roomId = resp.data.room_id;

      // 게시글 객체에 상대 이름 필드가 있다면 사용, 없다면 별도 호출 (여기선 post 정보 활용)
      ...
      // alert("채팅방 생성에 실패했습니다."); // 필요 시 주석 해제
    }
  };
