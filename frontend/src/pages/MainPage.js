...
import axios from "axios"; // isCancel 처리를 위해 필요 (api 모듈의 원본 axios)
...
function MainPage({ userId, userName }) {
    ...
    (async () => {
      try {
        ...
        const { data } = await api.get(
          `/api/users/${userId}/dashboard/?month=${monthParam}`,
          { signal: ctrl.signal }
        );
  ...
  const fetchTaskDetails = async (type) => {
      try {
        // ✅ api 모듈 사용
        const response = await api.get(`/api/users/task-details/?type=${type}`);
      const titles = {
        my: '내가 맡은 업무',
        incomplete: '미완료된 업무',
        feedback: '피드백 필요한 업무',
  ...
  const handleFavoriteToggle = async (projectId, isFavorite) => {
    ...
    const url = `/api/users/${userId}/projects/${projectId}/favorite/`;
                          ...
                          <div className="project-detail-inline">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                // ✅ api 모듈 사용 (JSON 전송)
                                api.post("/api/users/projects/set/", { project_id: project.project_id })
                          ...
                          {modalData.projects.map((p) => (
                            <div key={p.project_id} className="task-item">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  // ✅ api 모듈 사용
                                  api.post("/api/users/projects/set/", { project_id: p.project_id })
